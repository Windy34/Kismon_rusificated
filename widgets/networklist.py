...
class NetworkList:
    def __init__(self, networks, locate_network_on_map, on_signal_graph, config):
        ...
        self.config = config
        self.value_cache = {}
        for key in ('Время', 'Шифрование', 'Сервер', 'Тип', 'Канал', 'Сигнал', 'SSID'):
        ...
        self.columns = ("BSSID", "Тип", "SSID", "Канал", "Шифрование",
                        "Время обнаружения", "Последнее появление", "Широта", "Долгота",
                        "Уровень сигнала dbm", "Комментарий", "Серверы")
        self.available_columns = {}
        if len(self.config['network_list_columns']) == 0:
            self.config['network_list_columns'] = list(self.columns)
        for column in self.columns:
            renderer = Gtk.CellRendererText()
            if column == "Комментарий":
                renderer.set_property('editable', True)
                renderer.connect("editing-started", self.on_comment_editing_started)
            elif column == "Уровень сигнала dbm":
        ...
        locate_item = Gtk.MenuItem.new_with_label('Копировать поле')
        network_popup.append(locate_item)
        locate_item.connect("activate", self.on_copy_field)
        locate_item = Gtk.MenuItem.new_with_label('Копировать сеть')
        network_popup.append(locate_item)
        locate_item.connect("activate", self.on_copy_network)

        locate_item = Gtk.MenuItem.new_with_label('Найти на карте')
        network_popup.append(locate_item)
        locate_item.connect("activate", self.on_locate_marker)

        signal_item = Gtk.MenuItem.new_with_label('Граф сигнала')
    ...
    def on_comment_editing_done(self, widget):
        ...
        network['Комментарий'] = widget.get_text()
    ...
    def prepare_network_signal(self, value):
        ...
        self.value_cache['Сигнал'][value] = (signal, signal_strength)
    ...
    def add_network(self, mac):
        ...
        if network['Комментарий'] == '':
            comment = None
        else:
            comment = network['Комментарий']

        line = [mac,
                self.prepare_network_type(network["Тип"]),
                self.prepare_network_ssid(network["SSID"]),
                self.prepare_network_channel(network["Канал"]),
                self.prepare_network_crypt(network["Шифрование"]),
                self.prepare_network_time(network["Время обнаружения"]),
                self.prepare_network_time(network["Последнее появление"]),
                self.prepare_network_coordinate(network["Широта"]),
                self.prepare_network_coordinate(network["Долгота"]),
                signal,
                comment,
                self.prepare_network_servers(network["Серверы"]),
                signal_strength
                ]
        try:
