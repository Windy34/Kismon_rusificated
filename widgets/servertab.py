...
class ServerTab:
    def __init__(self, server_id, map, config, client_threads, client_start, client_stop, set_server_tab_label,
        ...
        connection_expander.set_label("Настройки")
        ...
        info_expander.set_label("Справка")
        ...
        gps_expander.set_label("Данные GPS")
        ...
        if self.map is not None:
            ...
            track_expander.set_label("Слежение GPS")
        ...
        sources_expander.set_label("Источники")
    ...
    def init_control_table(self):
        ...
        label = Gtk.Label(label='Изменить:')
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        table.attach(label, 0, 1, row, row + 1)
        ...
        remove_button = Gtk.ModelButton(label='Удалить сервер')
    ...
    def init_info_table(self, server_id):
        ...
        networks_label = Gtk.Label(label="Девайсы: ")
    ...
    def init_gps_table(self):
        ...
        lat_label = Gtk.Label(label="Широта: ")
        ...
        lon_label = Gtk.Label(label="Долгота: ")
    ...
    def init_track_table(self):
        ...
        label = Gtk.Label(label='Показать:')
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        table.attach(label, 0, 1, row, row + 1)
        ...
        label = Gtk.Label(label='Перейти к:')
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        table.attach(label, 0, 1, row, row + 1)
    ...
    def init_sources_table_source(self, source, table):
        ...
        rows.append(("Тип", source["type"]))
        rows.append(("Пакеты", source["packets"]))
    ...
    def update_sources_table(self, sources):
        ...
        for uuid in sources:
            ...
            sources_table_source["Тип"].set_text("%s" % source["type"])
            sources_table_source["Пакеты"].set_text("%s" % source["packets"])
    ...
    def on_server_edit(self, widget):
        ...
        dialog = Gtk.Dialog(title="Настроить сервер")
        ...
        label = Gtk.Label()
        label.set_markup("<b>Пароль*</b>: ")

        table.attach(label, 0, 1, row, row + 1)

        password_entry = Gtk.Entry()
        password_entry.set_visibility(False)
        password_entry.set_placeholder_text("Пароль")
        ...
        response = dialog.run()
        if response < 1:
            self.logger.info("Отмена (%s)" % response)
            dialog.destroy()
            return False
    ...
    def on_server_switch(self, widget):
        if widget.get_active():
            ...
            state = 'connected'
            icon = 'gtk-connect'
            widget.set_tooltip_text('Отключиться')
        else:
            self.on_server_disconnect(None)
            state = 'disconnected'
            icon = 'gtk-disconnect'
            widget.set_tooltip_text('Подключиться')

        self.set_server_tab_label(self.server_id, icon, "Сервер %s %s" % ((self.server_id + 1), state))
    ...
    def try_authentification(self):
        ...
        while not client.authenticate():
            ...
            if not response:  # if the dialog was canceled
                return False
            self.logger.info("Следующая попытка")
        return True
