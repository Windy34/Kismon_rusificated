...
class FilterTab:
    ...
    def add_limiter_radiobuttons(self, main_x, main_y):
        frame = Gtk.Frame()
        frame.set_label("Максимальное колличество сетей")
        ...
        for name, key in (("Network List", "network_list"), ("Map", "map"), ("Export", "export")):
            ...
            for text, value in (('Отключено', 'none'), ('Только текущая сессия', 'current'), ('Все сети', 'all')):
                x += 1
                if y == 1:
    ...
    def add_regex_filters(self, main_x, main_y):
        frame = Gtk.Frame()
        frame.set_label("Регулярное выражение")
        self.grid.attach(frame, main_x, main_y, 1, 1)
