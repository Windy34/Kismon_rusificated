...
class ConfigWindow:
    def __init__(self, main_window):
        ...
        self.gtkwin.set_title("Настройки")
    ...
    def init_general_page(self, page):
        ...
        label = Gtk.Label(label="Максимальное число записей в журнале: ")
        ...
        frame = Gtk.Frame()
        frame.set_label("Метки")
        page.attach(frame, 0, 1, 2, 3, yoptions=Gtk.AttachOptions.SHRINK)
        hbox = Gtk.HBox()
        frame.add(hbox)

        checkbox = Gtk.CheckButton.new_with_label("Сохранять GPS метки")
    ...
    def init_map_page(self, map_page):
        ...
        map_widget = Gtk.RadioButton(group=None, label='В главном окне ')
        ...
        map_window = Gtk.RadioButton(group=map_widget, label='Во вспомогательном окне')
        ...
        perf_frame.set_label("Настройки")
        perf_vbox = Gtk.VBox()
        perf_frame.add(perf_vbox)
        map_page.attach(perf_frame, 0, 1, 4, 5, yoptions=Gtk.AttachOptions.SHRINK)

        perf_marker_positions = Gtk.CheckButton.new_with_label("Обновить метки")
