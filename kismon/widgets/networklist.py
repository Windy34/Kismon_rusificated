...
class NetworkList:
    def __init__(self, networks, locate_network_on_map, on_signal_graph, config):
        ...
        self.columns = ("BSSID", "Тип", "SSID", "Канал", "Шифрование",
                        "Первое обнаружение", "Последнее обнаружение", "Широта", "Долгота",
                        "Уровень дБ", "Комментарий", "Сервер")
        ...
        locate_item = Gtk.MenuItem.new_with_label('Копировать запись')
        network_popup.append(locate_item)
        locate_item.connect("activate", self.on_copy_network)

        locate_item = Gtk.MenuItem.new_with_label('Показать на карте')
        network_popup.append(locate_item)
    ...
    def prepare_network_signal(self, value):
        ...
        """ Карты Wi-Fi дают разные диапазоны сигнала, некоторые дают от
        1xx до 0, а другие от 0 до 100. Для выполнения CellRendererProgress требуется
        процентное значение от 0 до 100, поэтому мы преобразуем значение, если это
        необходимо.
        """
        if -100 <= value <= 0:
    ...
    def add_network(self, mac):
        ...
        """ Gtk.ListStore преобразует каждое значение типа Python в свое
          Эквивалент GObject. Большинство функций prepare_network_* кэшируются
         и возвращают значения в виде объекта, это ускорит процесс, так как у нас есть
         много повторяющихся значений. Кроме того, значение None быстрее строки
         нулевого размера, поэтому мы заменяем его там, где это возможно.
        """
        ...
        if mac in self.network_iter:
            ...
            # придерживаться верхней части таблицы после добавления новой строки
    ...
    def on_treeview_clicked(self, treeview, event):
        ...
        if event.type == Gdk.EventType.DOUBLE_BUTTON_PRESS:  # двойное нажатие
            self.on_locate_marker(None)
        elif event.button == 3:  # правый клие
