...
class FilterTab:
    def __init__(self, config, networks, networks_queue_progress):
        ...
        self.add_checkbox_list(config_key='filter_crypt', title='Шифрование', kv=crypt_kv, x=x, y=y)
        ...
        x += 1
        self.add_checkbox_list(config_key='filter_type', title='Тип сети', kv=type_kv, x=x, y=y)
    ...
    def add_limiter_radiobuttons(self, main_x, main_y):
        frame = Gtk.Frame()
        frame.set_label("Максимум сетей")
        ...
        groups = {}
        for name, key in (("Список сетей", "network_list"), ("Карта", "map"), ("Экспортировать", "export")):
            x = 0
            label = Gtk.Label(label=name)
            label.set_property("xalign", 0)
            limiter_grid.attach(label, x, y, 1, 1)

            for text, value in (('Отключить', 'none'), ('Только в текущая сессия', 'current'), ('Все сети', 'all')):
                x += 1
                if y == 1:
    ...
    def add_regex_filters(self, main_x, main_y):
        frame = Gtk.Frame()
        frame.set_label("Введите регулярное выражение")
        self.grid.attach(frame, main_x, main_y, 1, 1)
