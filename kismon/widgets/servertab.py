...
class ServerTab:
    def __init__(self, server_id, map, config, client_threads, client_start, client_stop, set_server_tab_label,
        ...
        connection_expander.set_label("Управление")
        ...
        if self.map is not None:
            ...
            track_expander.set_label("Метка GPS")
    ...
    def init_control_table(self):
        ...
        label = Gtk.Label(label='Настройка:')
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        table.attach(label, 0, 1, row, row + 1)
    ...
    def init_info_table(self, server_id):
        ...
        networks_label = Gtk.Label(label="Устройства: ")
    ...
    def init_gps_table(self):
        ...
        fix_label = Gtk.Label(label="Фикс: ")
    ...
    def init_track_table(self):
        ...
        label = Gtk.Label(label='Сбросить:')
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        table.attach(label, 0, 1, row, row + 1)
    ...
    def on_server_edit(self, widget):
        ...
        dialog = Gtk.Dialog(title="Настройка подключения")
        ...
        username_entry.set_placeholder_text("Стандартный: admin")
        ...
        label = Gtk.Label()
        label.set_markup("<b>* Обязателен, находится в kismet_httpd.conf</b>")
        ...
        response = dialog.run()
        if response < 1:
            self.logger.info("Вызов окна отменен (%s)" % response)
            dialog.destroy()
            return False
    ...
    def try_authentification(self):
        ...
        while not client.authenticate():
            ...
            if not response:  # если диалоговое окно было отменено
                return False
