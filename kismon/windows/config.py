...
class ConfigWindow:
    def __init__(self, main_window):
        ...
        self.notebook.set_tab_label_text(general_page, "Общие")
        ...
        self.notebook.set_tab_label_text(map_page, "Карта")

        if self.map is None:
            label = Gtk.Label(label="Карта отключена")
    ...
    def init_general_page(self, page):
        frame = Gtk.Frame()
        frame.set_label("Список событий")
        ...
        label = Gtk.Label(label="Максимальное количество строк в списке событий: ")
        ...
        label = Gtk.Label(label="-1 = бесконечно 0 = отключено")
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        hbox.pack_start(label, False, False, 5)

        frame = Gtk.Frame()
        frame.set_label("Автосохранение")
        page.attach(frame, 0, 1, 1, 2, yoptions=Gtk.AttachOptions.SHRINK)
        vbox = Gtk.VBox()
        frame.add(vbox)
        hbox = Gtk.HBox()
        vbox.add(hbox)
        label = Gtk.Label(label="Сохранять сети каждые (в минутах)")
        hbox.pack_start(label, False, False, 5)
        ...
        label = Gtk.Label(label="0 = отключено")
        label.set_property("xalign", 0)
        label.set_property("yalign", 0.5)
        hbox.pack_start(label, False, False, 5)
        ...
        checkbox = Gtk.CheckButton.new_with_label("Хранить GPS-метки")
    ...
    def init_map_page(self, map_page):
        ...
        position_frame.set_label("Местоположение")
        ...
        map_widget = Gtk.RadioButton(group=None, label='В главном окне (стандартно)')
        ...
        map_window = Gtk.RadioButton(group=map_widget, label='В отдельном окне')
        ...
        map_hide = Gtk.RadioButton(group=map_widget, label='Спрятать')
        ...
        x = 1
        for name in ("     Уровни масштабирования: ", " - "):
