...
class ChannelWindow:
    def __init__(self, sources, client_thread, parent):
        ...
        self.gtkwin.set_title("Настройка канала")
    ...
    def init_box(self):
        ...
        for uuid in self.sources:
            ...
            frame.add(table)
            hop_button = Gtk.RadioButton.new_with_label_from_widget(None, 'Перепрыгивать')
            if source["hop"] > 0:
                hop_button.clicked()
            ...
            lock_button = Gtk.RadioButton.new_with_label_from_widget(hop_button, "Блокировка")
            if source["hop"] == 0:
                lock_button.clicked()
        ...
        cancel_button = Gtk.Button.new_with_mnemonic('_Отменить')
        ...
        update_button = Gtk.Button.new_with_mnemonic('_Обновить')
